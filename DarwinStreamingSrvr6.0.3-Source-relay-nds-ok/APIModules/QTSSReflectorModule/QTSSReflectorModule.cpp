...
QTSS_Error  QTSSReflectorModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
    switch (inRole)
    {
        ...
        {   qtss_printf("QTSSReflectorModuleDispatch ,QTSS_ClientSessionClosing_Role\n");
...
*/
void generate_sc_nds_sdp(char *sdp_buf,QTSS_StandardRTSP_Params* inParams,int port,UInt32 l_callid)
{
    ...
    err = QTSS_GetValuePtr(theRTSPSession, qtssRTSPSesLocalAddrStr, 0, (void **)&theRequestAttributes, &attributeLen);
        ...
        UInt32 len = sizeof(server_port);
        err = QTSS_GetValue(theRTSPSession, qtssRTSPSesLocalPort, 0, &server_port, &len); 
        
        sprintf(sdp_buf+strlen(sdp_buf),"v=0\r\no=- %s %lu IN IP4 %s\r\ns=\r\nt=0 0\r\na=control:rtsp://%s:%d/%s\r\nc=IN IP4 %s\r\nm=video %d MP2T/AVP/UDP\r\n",sessIDCStr,l_callid,serverHost,serverHost,server_port,sessIDCStr,serverHost,port);
        delete sessIDCStr;
}

//create video's sdp form database with vid
QTSS_Error  generate_sdp(char *vid,UInt32 l_callid,char *sdp_buf,Bool16 isCamera,char *src_host,UInt32 *src_port,char *local_host,QTSS_StandardRTSP_Params* inParams)
{
    ...
    {
        generate_sc_nds_sdp(sdp_buf,inParams,port,l_callid);
    }
   ...
   if(strstr(inPath->Ptr,"live") != NULL)
    ...
    if (theSessionRef == NULL)
    {
        ...
        {   
            ...
            if( generate_sdp(vid,l_callid,sdp_buf,isCamera,src_host,&src_port,local_host,inParams)<0 || src_port <= 0)
            {
                return NULL;
            }else
            {
				...
				//zlj add 1104
				char buf[32];
				qtss_sprintf(buf, "%"_S32BITARG_"", sdpData.Len-29);
				//(void)QTSS_AppendRTSPHeader(inParams->inRTSPRequest, qtssContentLengthHeader,&buf[0], ::strlen(&buf[0]));
...
/*zlj 
	(void)QTSS_AppendRTSPHeader(inParams->inRTSPRequest, qtssCacheControlHeader,
                                kCacheControlHeader.Ptr, kCacheControlHeader.Len);
*/
            char sdp_buf_r[4096]={'\0'};
            generate_sc_nds_sdp(sdp_buf_r,inParams,0,l_callid);
            StrPtrLen sdpStr_r(sdp_buf_r);
			char buf_r[32];
			qtss_sprintf(buf_r, "%"_S32BITARG_"", sdpStr_r.Len);
			(void)QTSS_AppendRTSPHeader(inParams->inRTSPRequest, qtssContentLengthHeader,&buf_r[0], ::strlen(&buf_r[0]));
