...
:   fIsSetup(false),
    ...
    fStartStoreTask(false),
    fStoreTask(NULL),
    fARTSRelayInfo(NULL),
    fStartSourceThread(false)
...
ReflectorSession::~ReflectorSession()
{
    ...
    if(fStoreTask)
    {
        fStoreTask->Signal(Task::kKillEvent);         
    }
...
QTSS_Error ReflectorSession::SetupReflectorSession(SourceInfo* inInfo, QTSS_StandardRTSP_Params* inParams, UInt32 inFlags, Bool16 filterState, UInt32 filterTimeout,ARTSRelayInfo *fARTSRelayInfo)
{   
    if (inInfo == NULL) // use the current SourceInfo
        inInfo = fSourceInfo;
    
    QTSS_RTPTransportType transportType = 0; 
    ...
    for (UInt32 x = 0; x < fSourceInfo->GetNumStreams(); x++)
    {
        ...
        if (theStreamRef == NULL)
        {
            ...
            if(fARTSRelayInfo != NULL)
            {
                fStreamArray[x] ->SetTCPDataSocket(fARTSRelayInfo->GetClientSocket()->GetTCPSocket()); 
                fStreamArray[x]->fRTSPClient = fARTSRelayInfo->GetRTSPClient();
                transportType = qtssRTPTransportTypeTCP; 
            }
                         
            QTSS_Error theError = fStreamArray[x]->BindSockets(inParams,inFlags, filterState, filterTimeout,transportType);
            if (theError != QTSS_NoErr)
            {   
                qtss_printf("BindSockets error\n");
                delete fStreamArray[x];
                fStreamArray[x] = NULL;
                return theError;
            }
            ...
            if(fARTSRelayInfo!=NULL)
            {
                qtss_printf("will set backupTask,%x\n",(ReflectorSocket*) fStreamArray[x]->GetSocketPair()->GetSocketA());
                fARTSRelayInfo->SetBackUpTask((ReflectorSocket*) fStreamArray[x]->GetSocketPair()->GetSocketA());
            }
            ...
            UInt32 refCount = fStreamArray[x]->GetRef()->GetRefCount();
            qtss_printf("SetupReflectorSession stream index=%"_U32BITARG_" refcount=%"_U32BITARG_"\n",x,refCount);
...
{
    qtss_printf("ReflectorSession::RemoveOutput\n");
...
{
    qtss_printf("ReflectorSession::TearDownAllOutputs\n");
    for (UInt32 y = 0; y < fSourceInfo->GetNumStreams(); y++)
