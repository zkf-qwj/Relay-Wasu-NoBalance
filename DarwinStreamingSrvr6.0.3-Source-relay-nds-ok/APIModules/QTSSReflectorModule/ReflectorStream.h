...
inline  SInt64  GetPacketNTPTime();
 ...
 private: 
        ...
        enum
        {
            kMaxReflectorPacketSize = 1320  //jm 5/02 increased from 2048 by 12 bytes for test bytes appended to packets
        };
...
class TCPInterleaveDemux 
{
    public:
        TCPInterleaveDemux(){fPacketLenIdx =kLenIdxUnknown;fPacketDataNeedLen =-1;}
        ~TCPInterleaveDemux(){}
        SInt32  GetPacketLenIdx() {return fPacketLenIdx;}
        SInt32  GetPacketDataNeedLen() {return fPacketDataNeedLen;}
        void    SetPacketLenIdx(SInt32 idx) {fPacketLenIdx = idx;}
        void    SetPacketDataNeedLen(SInt32 Len){fPacketDataNeedLen = Len;}
    enum
        {
            kLenIdxUnknown = -1  //jm 5/02 increased from 2048 by 12 bytes for test bytes appended to packets           
        };
    private:
        SInt32 fPacketLenIdx;//the position of the len of interleave tcp  packet in the whole paket in the whole packet
        SInt32 fPacketDataNeedLen;               
};
...
class ReflectorSocket : public IdleTask, public UDPSocket
{
    public:
        ...
        TCPSocket*   fTCPDataSocket ;
        RTSPClient * fRTSPClient;
        
        void    SetTransportType(QTSS_RTPTransportType TransportType ) { fTransportType = TransportType; }
               
        QTSS_RTPTransportType       GetTransportType()  { return fTransportType; }
        void GetMediaData(ReflectorPacket* thePacket);
    ...
    private:
       ...
       // Queue of senders
        ...
        char        fReceiveBuf[kMaxReceivePacketSize];
        StrPtrLen   fReceiveBufPtr;
       
        char        fTempBuf[204];
        UInt32      fTempBufDataLen;
               
        TCPInterleaveDemux *fLastTCPPacketDemux;       
};
...
class ReflectorSender : public UDPDemuxerTask
{
    ...
    OSQueueElem*    SendPacketsToOutput(ReflectorOutput* theOutput, OSQueueElem* currentPacket, SInt64 currentTime,  SInt64  bucketDelay, Bool16 firstPacket,QTSS_Error*sendErr);
...
class ReflectorStream
{
    public:
        ...
        QTSS_Error BindSockets(QTSS_StandardRTSP_Params* inParams, UInt32 inReflectorSessionFlags, Bool16 filterState, UInt32 timeout,QTSS_RTPTransportType InTransportType);
        ...
        TCPSocket*              GetTCPDataSocket()  { return fTCPDataSocket;}
        void                    SetTCPDataSocket(TCPSocket* fCurSocket)  { fTCPDataSocket = fCurSocket;}
