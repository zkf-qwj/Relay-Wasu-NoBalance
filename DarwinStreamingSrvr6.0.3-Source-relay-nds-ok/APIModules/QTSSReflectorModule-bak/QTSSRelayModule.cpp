...
{
    // Setup module utils
     // qtss_printf(">>>RelayModule Initialize\n");
...
QTSS_Error RereadPrefs()
{
 ...
 //qtss_printf(">>> ReadRelayPrefsFile\n");
    ReadRelayPrefsFile();
    return QTSS_NoErr;
}
...
QTSS_Error SetupAnnouncedSessions(QTSS_StandardRTSP_Params* inParams)
{
   ...
   // qtss_printf("SetupAnnouncedSessions  Entry\n");
   ...
   if ( (*theMethod == qtssSetupMethod))
    ...
    if(info == NULL)  
    {
       // qtss_printf("RTSPSourceInfo is null\n");
        // if a source info does not exists, we don't have to do anything
        return QTSS_NoErr;
    }
...
//              RTSPSourceInfo* theInfo = dynamic_cast<RTSPSourceInfo *>(theElem->fSourceInfo);
                ...
                //qtss_printf(">>>StartSessionCreatorTask\n");
