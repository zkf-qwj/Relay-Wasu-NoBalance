...
RTSPSession::~RTSPSession()
{
   ...
   //qtss_printf("finish ~RTSPSession\n");
}
...
SInt64 RTSPSession::Run()
{
    ...
    if ((events & Task::kTimeoutEvent) || (events & Task::kKillEvent)){     
        ...
        {
            qtss_printf("obj:%x,Timeout,time:%"_64BITARG_"d\n",this,QTSS_Milliseconds());
        }   else
        {
           qtss_printf("obj:%x,kKillEvent\n",this); 
        }
        fLiveSession = false;
    }
    Bool16 live = this->IsLiveSession();
    
    qtss_printf("obj:%x,livesess:%d,time:%"_64BITARG_"d\n",this,live,QTSS_Milliseconds());
    ...
    {		
       ...
       // qtss_printf("Rtspobj:%x,livesess:%d\n",this,this->IsLiveSession());
        qtss_printf("fState:%d,obj:%x\n",fState,this);
        switch (fState)
        {
            case ksendStreamingData:
                if(OtherRTSPSession != NULL)
                {
                    RTSPSession * otherObj = (RTSPSession *)OtherRTSPSession;
                    RTSPResponseStream *fOutputStream = otherObj->GetOutputStream();
                    err = fOutputStream->FlushData();
                    if(err == EAGAIN)
                    {
                        fOutputStream->GetDataSocket()->RequestEvent(EV_WR);                                               
                    }  
                }
                return 0;
            case kReadingFirstRequest:
            {
                 //qtss_printf("kReadingFirstRequest:,obj:%x\n",this);
            ...
            {   
                //qtss_printf("kHTTPFilteringRequest,obj:%x\n",this);
            ...
            case kReadingRequest:
            {
               ...
               // qtss_printf("kReadingRequest,obj:%x\,fRequest:%x\n",this,fRequest);
            ...
            {   
                ...
                //char test[1024]={'\0'};
               // memcpy(test,fInputStream.GetRequestBuffer()->Ptr,fInputStream.GetRequestBuffer()->Len);
               // qtss_printf("kHaveNonTunnelMessage:buffer:%s",test);
            ...
            case kFilteringRequest:
            {
                ...
                //qtss_printf("kFilteringRequest:,obj:%x\n",this);
                //
                ...
                {
                    //qtss_printf("why HandleIncomingDataPacket");
                    this->HandleIncomingDataPacket();
                  // qtss_printf("will goto cleanup,obj:%x\n",this);
                    fState = kCleaningUp;
                    break;
                }
                ...
                //qtss_printf("setuprequest begin,obj:%x\n",this);
                    this->SetupRequest();
                  //qtss_printf("setuprequest end,obj:%x\n",this);
                   if(fRTPSession == NULL)
                   {
                    qtss_printf("setuprequest failed, obj:%x\n",this);
                   }
                }
            ...
            case kRoutingRequest:
            {
               ...
               // qtss_printf("kRoutingRequest:,obj:%x\n",this);
                ...
                if(fRequest->SkipAuthorization())
                {
                    ...
                    // SETUP was issued on this same TCP connection.
                        fLastRTPSessionIDPtr.Len = 0;
                         qtss_printf("2 fLastRTPSessionIDPtr.Len:%d\n",fLastRTPSessionIDPtr.Len);
                    }
                    else if (fLastRTPSessionIDPtr.Len == 0)
                    {
                         qtss_printf("3 fLastRTPSessionIDPtr.Len:%d\n",fLastRTPSessionIDPtr.Len);
            ...
            {
               // qtss_printf("kAuthenticatingRequest,obj:%x\n",this);
            ...
            case kAuthorizingRequest:
            {
                ...
                //qtss_printf("kAuthorizingRequest\n");
            ...
            {    theModule->GetValue(qtssModName)->PrintStr("QTSSModule::CallDispatch ENTER module=", "\n");
                ...
                if (fRequest->GetMethod() == qtssSetupMethod && !(fRequest->GetIsMPEG2())){
                    // Make sure to erase the session ID stored in the request at this point.
                    // If we fail to do so, this same session would be used if another
                    // SETUP was issued on this same TCP connection.
                    fLastRTPSessionIDPtr.Len = 0;
                     qtss_printf("4 fLastRTPSessionIDPtr.Len:%d\n",fLastRTPSessionIDPtr.Len);}
                 else if (fLastRTPSessionIDPtr.Len == 0)
                 {
                    fLastRTPSessionIDPtr.Len = ::strlen(fLastRTPSessionIDPtr.Ptr); 
                     qtss_printf(" 5 fLastRTPSessionIDPtr.Len:%d\n",fLastRTPSessionIDPtr.Len);
                  }
            }
            ...
            case kPreprocessingRequest:
            {
                ...
                //qtss_printf("kPreprocessingRequest:,obj:%x\n",this);
                numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPPreProcessorRole);
                {
                    ...
                    for (; (fCurrentModule < numModules) && ((!fRequest->HasResponseBeenSent()) || fModuleState.eventRequested); fCurrentModule++)
                    {
                        ...
                        if (fRTPSession->HasAnRTPStream() && (fRTPSession->GetPacketSendingModule() == NULL) || fRequest->earlyPlayFlag)
                        ...
                        if (fModuleState.eventRequested)
                        {
                            this->ForceSameThread();    // We are holding mutexes, so we need to force
                            //fInputSocketP->RequestEvent(EV_RE);
                           // qtss_printf("this:%x,idel_Time:%d\n",this,fModuleState.idleTime);                       // the same thread to be used for next Run()
                            return fModuleState.idleTime; // If the module has requested idle time...
                        }
                    }
                }
                ...
                if (fRequest->HasResponseBeenSent())
                {
                   // qtss_printf("HasResponseBeenSent()\n");
                    fState = kPostProcessingRequest;
                    break;
                }
                fState = kProcessingRequest;
               //qtss_printf("kPreprocessingRequest: finished\n");
            }

            case kProcessingRequest:
            {
               // qtss_printf("kProcessingRequest,obj:%x\n",this);
                ...
                if (QTSServerInterface::GetNumModulesInRole(QTSSModule::kRTSPRequestRole) > 0)
                {
                   ...
                   // qtss_printf("callDispatch,this:%x\n",this);
                    ...
                    if (fModuleState.eventRequested)
                    {
                        //qtss_printf("this:%x,fModuleState.idleTime:%d\n",this,fModuleState.idleTime);
                        this->ForceSameThread();    // We are holding mutexes, so we need to force
                                               // the same thread to be used for next Run()
                        return fModuleState.idleTime; // If the module has requested idle time...
                    }
                    
                   // qtss_printf("Entry call Module,this:%x\n",this);
                }
                ...
                fState = kPostProcessingRequest;
              // qtss_printf("kProcessingRequest finished,obj:%x\n",this);
            }
            ...
            case kPostProcessingRequest:
            {
               ...
               //qtss_printf("kPostProcessingRequest,obj:%x\n",this);
            ...
            case kSendingResponse:
            {
                ...
                Assert(fRequest != NULL);
             //  qtss_printf("kSendingResponse,obj:%x\n",this);
            ...
            case kCleaningUp:
            {
              ...
              //  qtss_printf(" kCleaningUp,obj:%x\n",this);
                ...
                //qtss_printf(" run include CleanupRequest(),obj:%x\n",this);
                this->CleanupRequest();
    ...
    // at this point.
    //qtss_printf("total CleanupRequest(),obj:%x\n",this);
    this->CleanupRequest();

    // Only delete if it is ok to delete!
    qtss_printf("obj:%x,fObjHolders:%d\n",this,fObjectHolders);
    if (fObjectHolders == 0){
       // qtss_printf("fobj=0,will return -1,fd:%d\n",fOutputSocketP->GetSocketFD());
        fOutputSocketP->DontAutoCleanup();
        fOutputSocketP->Cleanup(); 
       
        return -1;
    }
    
    //if(fOutputSocketP->GetSocketFD() == -1)
    {
       // qtss_printf("tcp close ,release this rtspsession:%x",this);
        //return -1;
    }
...
void RTSPSession::sendSETPARA()
{

   StrPtrLen * cSeqPtr = fRequest->GetHeaderDictionary()->GetValue(qtssCSeqHeader);

    StrPtrLen *path = (StrPtrLen *) fRequest->GetValue(qtssRTSPReqAbsoluteURL);
    char *uri = path->Ptr;
    int seq =0;
    if(cSeqPtr->Ptr != NULL)
    {
          char * cseq = cSeqPtr->Ptr;
          seq = atoi(cseq);
          seq ++;
            
    }
        
    char * sessionID =fRequest->GetHeaderDictionary()->GetValue(qtssSessionHeader)->Ptr;
    ...
    qtss_printf("%s\n",annReq);
    StrPtrLen annReqStr(annReq);
    RTSPResponseStream *fOutputStream = this->GetOutputStream();
...
void RTSPSession::SetupRequest()
{
   ...
   // let's also refresh RTP session timeout so that it's kept alive in sync with the RTSP session.
    ...
    if(fRequest->GetMethod() == qtssPingMethod)
    {

       //qtss_printf("qtssPingMethod\n");
       statusCode = qtssSuccessOK;
       fRequest->SetValue(qtssRTSPReqStatusCode, 0, &statusCode, sizeof(statusCode));
       fRequest->SendHeader();
           return;
    }
    ...
    /*
    if(fRequest->GetMethod() == qtssValidateMethod)
    {
    
       qtss_printf("qtssValidateMethod\n");
       statusCode = qtssSuccessOK;
       fRequest->SetValue(qtssRTSPReqStatusCode, 0, &statusCode, sizeof(statusCode));
       fRequest->SendHeader();
	   return;
...
{
	//modified by lijie, 2010.10.17
    // Invoke the session closing modules
	//in case that rtsp session is established by Describe request (adapter pipeline created), 
	//but disconnected by client before send SETUP request (no rtp session established).
	if (fLiveSession && fRTPSession == NULL)
	{
		QTSS_RoleParams theParams;
		theParams.rtspSessionClosingParams.inRTSPSession = this;
		...
		//qtss_printf("kRTSPSessionClosingRole,obj:%x\n",this);
    }
	//jieli
    ...
    if (fRequest != NULL)
    {
       ...
       // qtss_printf("delete 3\n");
        delete fRequest;
        //qtss_printf("delete fRequest ok\n");
        fRequest = NULL;
        fRoleParams.rtspRequestParams.inRTSPRequest = NULL;
    ...
    //qtss_printf("exiting cleanuprequest,fRequest:%x\n",fRequest);
}
...
QTSS_Error  RTSPSession::FindRTPSession(OSRefTable* inRefTable)
{
    ...
    qtss_printf("fLastRTPSessionIDPtr.Len:%d\n",fLastRTPSessionIDPtr.Len);
    ...
    
    {
        ...
        while (activationError == EPERM)
        {
            ...
            //If that's the case, we'll simply retry until we get a unique ID
             qtss_printf(" 1 fLastRTPSessionIDPtr.Len:%d\n",fLastRTPSessionIDPtr.Len);
...
QTSS_Error RTSPSession::IsOkToAddNewRTPSession()
{
    ...
    {
        qtss_printf("OverMaxConnections,return 453\n");
    ...
    {
        qtss_printf("GetMaxKBitsBandwidth reached, return453\n");
        return QTSSModuleUtils::SendErrorResponse(fRequest, qtssClientNotEnoughBandwidth,
