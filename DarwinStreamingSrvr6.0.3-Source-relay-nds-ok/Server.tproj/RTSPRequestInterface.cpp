...
void  RTSPRequestInterface::WriteContext(StrPtrLen *str)
{
    if(str && str->Len >0 && str->Ptr != NULL)
    {
        fOutputStream->Put(*str);
    }
}
...
qtss_printf("zlj -- outFirstTransport.Ptr:%s\n",outFirstTransport.Ptr);
    ...
    if (stripClientPortStr.Len != 0)
        ...
        qtss_printf("zlj -- fOutputStream->GetBufPtr():%s\n",fOutputStream->GetBufPtr()); 
...
// Chris-Burns@yahoo.com (2009/03/19) [next 1 line/s]
    ...
    if (serverPortA != NULL && isMPEG2 == false)
    {
...
void RTSPRequestInterface::WriteStandardHeaders()
{
    ...
    if (fStatus == qtssSuccessOK)
    {
        ...
        fOutputStream->PutEOL();
	//zlj add  method 
	if (this->GetMethod() == qtssSetupMethod){
		StrPtrLen method_str("Method-Code: SETUP\r\n");
		fOutputStream->Put(method_str);
	}else if (this->GetMethod() == qtssPlayMethod){
		StrPtrLen method_str("Method-Code: PLAY\r\n");
		fOutputStream->Put(method_str);
	}else if (this->GetMethod() == qtssPauseMethod){
	
		StrPtrLen method_str("Method-Code: PASUE\r\n");
		fOutputStream->Put(method_str);
	}else if (this->GetMethod() == qtssOptionsMethod){
		StrPtrLen method_str("Method-Code: OPTIONS\r\n");
		fOutputStream->Put(method_str);
	
	}else if (this->GetMethod() == qtssGetParameterMethod){
		StrPtrLen method_str("Method-Code: GET_PARAMETER\r\n");
		fOutputStream->Put(method_str);
	
	}else if (this->GetMethod() == qtssSetParameterMethod){
		StrPtrLen method_str("Method-Code: SET_PARAMETER\r\n");
		fOutputStream->Put(method_str);
	
	}else if (this->GetMethod() == qtssValidateMethod){
		StrPtrLen method_str("Method-Code: VALIDATE\r\n");
		fOutputStream->Put(method_str);
	}else if (this->GetMethod() == qtssTeardownMethod){
		StrPtrLen method_str("Method-Code: TEARDOWN\r\n");
		fOutputStream->Put(method_str);
	}
	

	this->AppendDateAndExpires();//zlj add date
