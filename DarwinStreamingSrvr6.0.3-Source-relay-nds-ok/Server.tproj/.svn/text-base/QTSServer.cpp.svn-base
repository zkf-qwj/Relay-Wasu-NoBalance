...
void QTSServer::StartTasks()
{
    ...
    fStatsTask = new RTPStatsUpdaterTask(); //orig-darwin has this code,but no used ,can cause memleak, discast it by lili
...
void    QTSServer::LoadCompiledInModules()
{
    ...
    QTSSModule* theARTSRTSPModule = new QTSSModule("ARTSRTSPModule");
    (void)theARTSRTSPModule->SetupModule(&sCallbacks, &ARTSRTSPModule_Main);
    (void)AddModule(theARTSRTSPModule);	
...
void    QTSServer::InitCallbacks()
{
    ...
    sCallbacks.addr[kEnableDiffServ] =              (QTSS_CallbackProcPtr)QTSSCallbacks::QTSS_Enable_DiffServ;
...
UDPSocketPair*  RTPSocketPool::ConstructUDPSocketPair()
{
    ...
    return NEW
        UDPSocketPair(  NEW UDPSocket(theTask, Socket::kNonBlockingSocketType),NEW UDPSocket(theTask, UDPSocket::kWantsDemuxer | Socket::kNonBlockingSocketType));
}
