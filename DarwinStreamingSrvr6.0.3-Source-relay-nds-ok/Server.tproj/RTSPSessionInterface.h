...
public:
    ...
    //fast as possible
    Bool16 IsLiveSession()      { 
                                    //Bool16 constate = fSocket.IsConnected();
                                    //qtss_printf("obj:%x,fd:%d,constate:%d\n",this,fSocket.fFileDesc,constate);
                                    return fSocket.IsConnected() && fLiveSession; }
    ...
    void IncrementObjectHolderCount() { (void)atomic_add(&fObjectHolders, 1); }
     void cleanObjectHolderCount(bool flag) { if(flag){ fObjectHolders = 0;}else{  fObjectHolders = 1;}qtss_printf("this:%x cleanObjectHolderCount,fObjectHolders:%d\n",this,fObjectHolders);}
    void DecrementObjectHolderCount();
    Bool16                  supportOtherChannel;
    ...
    void                SetTimeOut(SInt64 timeOutInMS) { fTimeoutTask.SetTimeout(timeOutInMS);}
   // void                SetignoreTimeout() { ignoreTimeOut = true; }             
    ...
    QTSS_Error  InterleavedWrite(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, unsigned char channel,UInt32 sendType);
	...
	void        SetSupportOtherChannel() { supportOtherChannel = true;}
	
	
	Bool16      GetSupportOtherChannel()            { return supportOtherChannel;}
	Bool16      szModule;
	
	Bool16      NoCleanObjectHolderCount;
	
	Bool16		SentOptionsRequest()                { return fSentOptionsRequest; }
	SInt32		RoundTripTime()                     { return fRoundTripTime; }
	void        SetAdapterClose( Bool16 flag)       {  adapter_close = flag;}
	Bool16      adapter_close;
