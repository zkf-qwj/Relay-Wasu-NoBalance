...
:   QTSSDictionary(QTSSDictionaryMap::GetMap(QTSSDictionaryMap::kRTSPSessionDictIndex)),
	...
	adapter_close(false),NoCleanObjectHolderCount(false)
{
   ...
   // ignoreTimeOut = false;
    ...
    //fInputStream.ShowProtocolLog(QTSServerInterface::GetServer()->GetPrefs()->IsProtocolLogEnabled());
    //fOutputStream.ShowProtocolLog(QTSServerInterface::GetServer()->GetPrefs()->IsProtocolLogEnabled());

    //fInputStream.SetProtocolLogFile(QTSServerInterface::GetServer()->GetPrefs()->GetProtocolLogFileName());
    //fOutputStream.SetProtocolLogFile(QTSServerInterface::GetServer()->GetPrefs()->GetProtocolLogFileName());	
    supportOtherChannel=false;
    szModule = false;
}
...
RTSPSessionInterface::~RTSPSessionInterface()
{
    ...
    //qtss_printf("entry  ~RTSPSessionInterface(),obj:%x\n",this);
    if (fInputSocketP != fOutputSocketP) 
    {
       // qtss_printf("delete socket\n");
...
//maybe don't need this special case but for now on Win32 we do it the old way since the killEvent code hasn't been verified on Windows.
    ...
    if (0 == atomic_sub(&fObjectHolders, 1)){
        ...
        //qtss_printf("rtpclose , RTSP will close,this:%x",this);
...
QTSS_Error RTSPSessionInterface::InterleavedWrite(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, unsigned char channel,UInt32  sendType)
{
        ...
        )
    {
        ...
        err = this->GetOutputStream()->WriteV( iov, 2, fNumInCoalesceBuffer, &buffLenWritten, (RTSPResponseStream::kAllOrNothing)|sendType );
    ...
    if ( err == QTSS_NoErr )
    {
        ...
        if ( inLen > kTCPCoalesceDirectWriteSize )  
        {
            ...
            if(this->supportOtherChannel == true)
            {
                rih.len = (UInt16)inLen;   //yangzhou-hd need
            ...
            err = this->GetOutputStream()->WriteV( iov, 3, inLen + sizeof(rih), outLenWritten, (RTSPResponseStream::kAllOrNothing)|sendType );
