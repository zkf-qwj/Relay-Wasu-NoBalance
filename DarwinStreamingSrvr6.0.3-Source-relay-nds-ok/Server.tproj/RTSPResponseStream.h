...
class RTSPResponseStream : public ResizeableStringFormatter
{
    public:
        ...
        RTSPResponseStream(TCPSocket* inSocket, TimeoutTask* inTimeoutTask)
            :   ResizeableStringFormatter(fOutputBuf, kOutputBufferSizeInBytes),
                fSocket(inSocket), fBytesSentInBuffer(0), fTimeoutTask(inTimeoutTask),fPrintRTSP(false),fBytesSentInDataBuf(0),fDataBufFormatter(NULL,0){fDataSocket=NULL;}
        
        virtual ~RTSPResponseStream() {fDataSocket = NULL;}
        ...
        QTSS_Error Flush();
        QTSS_Error FlushData();
        
        void        ShowRTSP(Bool16 enable) {fPrintRTSP = enable; }  
        TCPSocket*  GetSocket(){ return  fSocket;}   

		void		 ShowProtocolLog(Bool16 enable) {fProtocolLogEnabled = enable; }   
		void		SetProtocolLogFile(char *inProtocolLogFileName) {	fProtocolLogFileName.Ptr = inProtocolLogFileName;
																		fProtocolLogFileName.Len = ( (inProtocolLogFileName != NULL) ? strlen(inProtocolLogFileName) : 0);}
        
        void       SetDataSocket(TCPSocket*  inSocket){fDataSocket =inSocket;}
        TCPSocket* GetDataSocket()  { return  fDataSocket;}
        SInt64     GetLeftDataLen();
    ...
    private:
        ...
        enum
        {
            ...
            kOutputDataBufferSizeInBytes = 1320
        };
		...
		Bool16					fProtocolLogEnabled;  // enable Protocol Logs 
		StrPtrLen				fProtocolLogFileName;
		TCPSocket*              fDataSocket;
		//char*                   fDataOutputBuf[kOutputDataBufferSizeInBytes];
		UInt32                  fBytesSentInDataBuf;
        ResizeableStringFormatter  fDataBufFormatter;
