...
void RTPSession::Teardown()
{
    ...
    if (fRTSPSession != NULL)
    {
        qtss_printf("Teardown,DecrementObjectHolderCount:%x,%"_64BITARG_"d\n",fRTSPSession,QTSS_Milliseconds());
        if(fRTSPSession->NoCleanObjectHolderCount == true ||  fRTSPSession->szModule == true)
        {
...
void    RTPSession::SendDescribeResponse(RTSPRequestInterface* inRequest)
{
    ...
    if ( inRequest->GetContentType() == qtssRTSPMHType &&  DoSessionSetupResponse(inRequest) != QTSS_NoErr)
      return;
