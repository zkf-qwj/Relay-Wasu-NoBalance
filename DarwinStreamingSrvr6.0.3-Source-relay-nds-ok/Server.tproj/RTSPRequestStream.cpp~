...
:   fSocket(sock),
    ...
    fRequestPtr(NULL),
    fDecode(false),
    fPrintRTSP(false),fProtocolLogEnabled(false)
{}
...
/*		
		if (fProtocolLogEnabled)
		{
			char*	filePath = fProtocolLogFileName.Ptr;	  
			FILE*	ProtocolFile = ::fopen(filePath, "a");
			...
			if (ProtocolFile != NULL)
			{
    	        ...
    	        if (fSocket != NULL)    
        	    {
                	...
                	if (theLocalAddrStr != NULL)
                	{	
                		qtss_fprintf(ProtocolFile,"#server: ip="); theLocalAddrStr->PrintStr(ProtocolFile); qtss_fprintf(ProtocolFile," port=%u\n" , serverPort );
                	}
                	else
              		{	
              			qtss_fprintf(ProtocolFile,"#server: ip=NULL port=%u\n" , serverPort );
              		}
               	
                	if (theRemoteAddrStr != NULL)
                	{	
                		qtss_fprintf(ProtocolFile,"#client: ip="); theRemoteAddrStr->PrintStr(ProtocolFile); qtss_fprintf(ProtocolFile," port=%u\n" , clientPort );
                	}
            		else
            		{
            			qtss_fprintf(ProtocolFile,"#client: ip=NULL port=%u\n" , clientPort );
            		}

            	}

				StrPtrLen str(fRequest);
				//qtss_fprintf(ProtocolFile,str.Ptr);
				str.PrintStrEOL("\n\r\n", "\n", ProtocolFile);// print the request but stop on \n\r\n and add a \n afterwards.
				::fclose(ProtocolFile);
			}
		}*/		
