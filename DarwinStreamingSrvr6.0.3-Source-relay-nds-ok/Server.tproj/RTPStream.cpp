...
{   /*fields:   fAttrName, fFuncPtr, fAttrDataType, fAttrPermission */
    ...
    /* 1  */ { "qtssRTPStrSSRC",                    NULL,   qtssAttrDataTypeUInt32, qtssAttrModeRead | qtssAttrModePreempSafe | qtssAttrModeWrite}, 
...
QTSS_Error RTPStream::Setup(RTSPRequestInterface* request, QTSS_AddStreamFlags inFlags)
{
    ...
    fTransportType = request->GetTransportType();	
     ...
     Bool16 IsMPEG2 = request->GetIsMPEG2();
    Bool16 isIpqam = request->GetIsIpqam();

    {
	     qtss_printf("SetUp transport type = %d,isMPEG2=%d,isIpqam=%d\n",fTransportType,IsMPEG2,isIpqam);
	}
    ...
    {
        qtss_printf("qtssRTPTransportTypeTCP\n");
        fIsTCP = true;
    ...
    qtss_printf("fRemoteRTPPort:%d,fRemodteAddr:%x\n",fRemoteRTPPort,fRemoteAddr);
    ...
    {		
     ...
     {
		 //As in case of Mpeg2 we are expecting only one port
         if (fRemoteRTPPort == 0 && isIpqam == false){
 ...
 //       return QTSSModuleUtils::SendErrorResponse(request, qtssClientBadRequest, qtssMsgRTCPPortMustBeOneBigger);       
    ...
    if (SocketUtils::IsMulticastIPAddr(fRemoteAddr))
    {
        ...
        //qtss_printf("The ip is MulticastIp\n");   
        if (fSockets != NULL)
        {
			if(fTransportType != qtssRTPTransportTypeMPEG2 && IsMPEG2 == false)
            {
...
void RTPStream::AppendTransport(RTSPRequestInterface* request)
{
    ...
    Bool16 isMPEG2 = request->GetIsMPEG2();
    StrPtrLen *inSessionID = fSession->GetValue(qtssCliSesRTSPSessionID);
...
// Chris-Burns@yahoo.com (2009/03/19) [next 7 line/s]
        ...
        if(request->IsPushRequest())
        {
            ...
            request->AppendTransportHeader(&rtpSPL, &rtcpSPL, NULL, NULL, &theSrcIPAddress,ssrcPtr,inSessionID);
        }       
        else
        {
            // Append UDP socket port numbers.
            ...
            request->AppendTransportHeader(theRTPSocket->GetLocalPortStr(), theRTCPSocket->GetLocalPortStr(), NULL, NULL, &theSrcIPAddress,ssrcPtr,inSessionID);
        }
    }
        ...
        request->AppendTransportHeader(NULL, NULL, &sChannelNums[fRTPChannel],  &sChannelNums[fRTCPChannel],NULL,ssrcPtr,inSessionID);
    else
    {
        ...
        request->AppendTransportHeader(NULL, NULL, &rtpChannel, &rtcpChannel,NULL,ssrcPtr,inSessionID);
    }
}
...
QTSS_Error  RTPStream::InterleavedWrite(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, unsigned char channel,UInt32 sendType)
{
    ...
    QTSS_Error err = fSession->GetRTSPSession()->InterleavedWrite( inBuffer, inLen, outLenWritten, channel,sendType);
...
//      qtss_printf("Flow controlled\n");
    ...
    else
    {
        ...
        qtss_printf("[RTPStream]ReliableRTPWrite\n");
...
QTSS_Error  RTPStream::Enable_DiffServ(UInt8 TOS){
  
  Assert(fSession != NULL);
  
  (void)fSockets->GetSocketA()->Enable_DiffServ(TOS);
  (void)fSockets->GetSocketB()->Enable_DiffServ(TOS);
...
QTSS_Error  RTPStream::Write(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, UInt32 inFlags)
{
    ...
    thePacket = (QTSS_PacketStruct*)inBuffer;
    ...
    {   
		qtss_printf("qtssWriteFlagsIsRTCP\n");
        ...
        {
            err = this->InterleavedWrite( thePacket->packetData, inLen, outLenWritten, fRTCPChannel,inFlags );
        }
        else if ( inLen > 0 )
        {
    ...
    {
       //qtss_printf("qtssWriteFlagsIsRTP\n");
        ...
        {
            if ( fTransportType == qtssRTPTransportTypeTCP )    // write out in interleave format on the RTSP TCP channel.
                err = this->InterleavedWrite( thePacket->packetData, inLen, outLenWritten, fRTPChannel,inFlags );       
            else if ( fTransportType == qtssRTPTransportTypeReliableUDP )
        ...
        if ( err == QTSS_NoErr && inLen > 0 )
        {
                ...
                (theTime > (fLastSenderReportTime + (senderReportInterval * 1000))))
            {
                ...
                qtss_printf("SendRTCPSR\n");
                this->SendRTCPSR(thePacket->packetTransmitTime);
            }
            
        }
    }
    else   if (inFlags & qtssWriteFlagsIsMpeg2)
    {         
            // Check to see if this packet fits in the overbuffer window
       thePacket->suggestedWakeupTime = fSession->GetOverbufferWindow()->CheckTransmitTime(thePacket->packetTransmitTime, theTime, inLen);
       // qtss_printf("qtssWriteFlagsIsMpeg2\n");
       
        if (thePacket->suggestedWakeupTime > theTime)
        {
           // Assert(thePacket->suggestedWakeupTime >= fSession->GetOverbufferWindow()->GetSendInterval());
#if RTP_PACKET_RESENDER_DEBUGGING
            fResender.logprintf("Overbuffer window full. Num bytes in overbuffer: %d. Wakeup time: %qd,suggest:%qd,theTime:%qd\n",fSession->GetOverbufferWindow()->AvailableSpaceInWindow(), thePacket->packetTransmitTime,thePacket->suggestedWakeupTime,theTime);
#endif
           
            qtss_printf("Overbuffer window full. Num bytes in overbuffer: %d. Wakeup time: %qd,suggest:%qd,theTime:%qd\n",fSession->GetOverbufferWindow()->AvailableSpaceInWindow(), thePacket->packetTransmitTime,thePacket->suggestedWakeupTime,theTime);
        ...
        if ( fTransportType == qtssRTPTransportTypeTCP ){    // write out in interleave format on the RTSP TCP channel.
                ...
                err = this->InterleavedWrite( thePacket->packetData, inLen, outLenWritten, fRTPChannel,inFlags ); 
                //qtss_printf("InterleavedWrite inFlags:%d\n",inFlags);
        }  
        else if ( fTransportType == qtssRTPTransportTypeReliableUDP ){
		...
		{		
                (void)fSockets->GetSocketA()->SendTo(fRemoteAddr, fRemoteRTPPort, thePacket->packetData, inLen);
                //qtss_printf("udp sendto,fRemoteAdde:%d,fRemoteRTPPort:%d\n",fRemoteAddr,fRemoteRTPPort); 
                //this->UDPMonitorWrite(thePacket->packetData, inLen, kIsRTPPacket);
		}
         ...
         {
               //qtss_printf("write packet failed,theErr:%d\n",err);
         }           
...
void RTPStream::SendRTCPSR(const SInt64& inTime, Bool16 inAppendBye)
{
    ...
    {
       UInt32  wasWritten; 
       err = this->InterleavedWrite( theSR->GetSRPacket(), thePacketLen, &wasWritten, fRTCPChannel,0 );
    }
    else
    {
