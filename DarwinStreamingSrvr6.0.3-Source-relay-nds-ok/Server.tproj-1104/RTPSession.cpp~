...
RTPSession::~RTPSession()
{
    ...
    {
        qtss_printf("OSMutexLocker theLocker(theServer->GetMutex())\n");
        ...
        qtss_printf("remove qtssSvrClientSessions\n");
        UInt32 y = 0;
...
*/
#endif
    qtss_printf("~RTPSession finished");
}
...
void RTPSession::SendDefaultResponse(RTSPRequestInterface* inRequest)
{
...
#endif
   // qtss_printf("RTPSession %X: In Play, about to call Signal\n",this);
    this->Signal(Task::kStartEvent);
    //qtss_printf("after call signal\n");
    return QTSS_NoErr;
}
...
void RTPSession::Teardown()
{
    ...
    if (fRTSPSession != NULL)
    {
        qtss_printf("Teardown,DecrementObjectHolderCount:%x\n",fRTSPSession);
        if(fRTSPSession->szModule == true)
        {
           fRTSPSession->cleanObjectHolderCount(false);
        }
...
void    RTPSession::SendDescribeResponse(RTSPRequestInterface* inRequest)
{
    ...
    if (DoSessionSetupResponse(inRequest) != QTSS_NoErr)
       return;
    ...
    static StrPtrLen sContentTypeSDP("application/sdp");
    
    
    static StrPtrLen sContentTypeRTSP("application/x-rtsp-mh");
    if(inRequest->GetContentType() == qtssRTSPMHType)
    {
        inRequest->AppendHeader(qtssContentTypeHeader, &sContentTypeRTSP);
    }
     
    if(inRequest->GetContentType() == qtssSDPType)
    {
        inRequest->AppendHeader(qtssContentTypeHeader, &sContentTypeSDP);
    }
...
void    RTPSession::SendAnnounceResponse(RTSPRequestInterface* inRequest)
{
    ...
    static StrPtrLen xnotice("EOS");
    inRequest->AppendHeader(qtssXAcceptDynamicRateHeader, &xnotice);
    (void)inRequest->SendHeader();
}

void    RTPSession::SendGetParameterResponse(RTSPRequestInterface* inRequest)
{
    //
    // Currently, no need to do anything special for a get parameter response
    (void)inRequest->SendHeader();


//    write x-Accept-Dynamic-Rate header
//    static StrPtrLen noticeStr("2101");
//    inRequest->AppendHeader(qtssXNoticeHeader, &noticeStr);
    
}

SInt64 RTPSession::Run()
{
    // qtss_printf("Entring Run,%"_64BITARG_"d\n",QTSS_Milliseconds());
#if DEBUG
    qtss_printf("start  Assert(fActivateCalled)\n");
    Assert(fActivateCalled);
    qtss_printf("after  Assert(fActivateCalled)\n");
#endif
    ...
    qtss_printf("RTPSession %X: In Run. Events %X,time:%"_64BITARG_"d\n",this,events,QTSS_Milliseconds());
    ...
    if ((events & Task::kKillEvent) || events & Task::kTimeoutEvent || (fModuleDoingAsyncStuff))
    {
        if (!fModuleDoingAsyncStuff)
        {
            if (events & Task::kTimeoutEvent)
            {
                ...
                //qtss_printf("RTPsess TimeOUt,this:%x,time:%"_64BITARG_"d\n",this,QTSS_Milliseconds());
            }
...
//#if RTPSESSION_DEBUGGING
       // qtss_printf("RTPSession %x: about to be killed. Eventmask = %"_S32BITARG_"\n",this, (SInt32)events);
//#endif
        ...
        // give the RTSPSessionTask a chance to release the RTPSession.
            ...
            if (!sessionTable->TryUnRegister(&fRTPMapElem))
            {
                ...
                //qtss_printf("RTP killEvent,this:%x,time:%"_64BITARG_"d\n",this,QTSS_Milliseconds());
        ...
        UInt32 numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kClientSessionClosingRole);
        {
            ...
            {  
                ...
                qtss_printf("start closesession,rtpsession:%x,time:%"_64BITARG_"d\n",QTSS_Milliseconds());
       ...
       qtss_printf("rtspsession:%x will return -1\n");
    ...
    #endif
            //qtss_printf("RTPSession %X: about to call SendPackets\n",this);
