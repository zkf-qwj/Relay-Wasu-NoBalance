...
RTSPSession::~RTSPSession()
{
    ...
    {
        QTSSModule* module =  QTSServerInterface::GetModule(QTSSModule::kRTSPSessionClosingRole, x);
        if( module)
        {
            module ->CallDispatch(QTSS_RTSPSessionClosing_Role, &theParams);
        }
    }
...
Bool16 RTSPSession::ParseOptionsResponse()
{
	...
	printf("zlj/add for qingyang theProtocol:%s\n",theProtocol.Ptr);	
...
void RTSPSession::SetupRequest()
{
   ...
   // let's also refresh RTP session timeout so that it's kept alive in sync with the RTSP session.
    ...
    if (fRequest->GetMethod() == qtssOptionsMethod)
    {
		...
		printf("zlj/add for qingyang ++++before:%s\n++++\n",fOutputStream.GetBufPtr());
		printf("zlj/add for qingyang options PublicHeader:%s,Len:%d\n",(QTSServerInterface::GetPublicHeader())->Ptr,(QTSServerInterface::GetPublicHeader())->Len);
        // DJM PROTOTYPE
        ...
        if ( requirePtr && requirePtr->EqualIgnoreCase(RTSPProtocol::GetHeaderString(qtssXRandomDataSizeHeader)) )
        {
			...
			printf("zlj/add for qingyang options 1\n");
        } 
		
		
		printf("zlj/add for qingyang fRequest:%x\n",fRequest);
		fRequest->SendHeader();
		printf("zlj/add for qingyang -------fOutputStream.GetBufPtr():%s\n----\n",fOutputStream.GetBufPtr());
	    
	    // now write the body if there is one
        if (bodySizeBytes > 0 && body != NULL){
            fRequest->Write(body, bodySizeBytes, NULL, 0);
			printf("zlj/add for qingyang options 2\n");
		}
        return;
    }
