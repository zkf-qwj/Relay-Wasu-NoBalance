...
RTSPProtocol::GetMethod(const StrPtrLen &inMethodStr)
{
    ...
    switch(*inMethodStr.Ptr)
    {
		...
		case 'G':   case 'g':   theMethod = qtssGetParameterMethod; break;	
		case 'V':   case 'v':   theMethod =  qtssValidateMethod;   break;	
    }
...
StrPtrLen RTSPProtocol::sHeaders[] =
{
    ...
    StrPtrLen("x-playNow"),
    StrPtrLen("x-wap-profile"),
    StrPtrLen("x-wap-profile-diff"),
    StrPtrLen("x-regionid"),
    StrPtrLen("x-notice"),
    StrPtrLen("X-position"),
    StrPtrLen("x-Ts-info"),
    StrPtrLen("ClientSessionId"),
    StrPtrLen("OnDemandSessionId")  
	
};

QTSS_RTSPStatusCode RTSPProtocol::GetQTSS_RTSPStatusCode(SInt32 sCode)
{
    QTSS_RTSPStatusCode l_StatusCode = 0;
    for(int i=0;i<qtssNumStatusCodes;i++)
    {
        if(sCode == sStatusCodes[i])
                l_StatusCode=i;
    }
    return l_StatusCode;
}
