...
{
	//qtss_printf("Parse Entring\n");

	/*zlj/add for qingyang 0921*/
#if 1
	if ((strstr(this->GetValue(qtssRTSPReqFullRequest)->Ptr,"SETUP") != NULL) && (strstr(this->GetValue(qtssRTSPReqFullRequest)->Ptr,"*") != NULL)){
		char *theRequestAttributes = this->GetValue(qtssRTSPReqFullRequest)->Ptr;
		if (theRequestAttributes != NULL){
			...
			if (start != NULL && end != NULL)
			{
				...
				//strncpy(newstr+len+strlen(fullstr),"\r\n",strlen("\r\n"));
				strncpy(theRequestAttributes,newstr,strlen(newstr));
			}
			this->GetValue(qtssRTSPReqFullRequest)->Len = strlen(newstr);
	qtss_printf("zlj/add=-----after this->GetValue(qtssRTSPReqFullRequest)->Len:%d\n",strlen(newstr));
	qtss_printf("zlj/add------after------------this->GetValue(qtssRTSPReqFullRequest)->Ptr:%s\n",this->GetValue(qtssRTSPReqFullRequest)->Ptr);
		}
	}
#endif
	StringParser parser(this->GetValue(qtssRTSPReqFullRequest));
	Assert(this->GetValue(qtssRTSPReqFullRequest)->Ptr != NULL);
	//qtss_printf("parse normal\n");
	//parse status line.
	QTSS_Error error = ParseFirstLine(parser);

	//handle any errors that come up    
	if (error != QTSS_NoErr){
		//qtss_printf("ParseFirstLine error:%d\n",error);
		return error;
	}  
	error = this->ParseHeaders(parser);
	if (error != QTSS_NoErr){
		//qtss_printf("ParseHeaders error:%d\n",error);
		return error;
	}
	...
	if (this->GetValue(qtssRTSPReqFilePath)->Len == 0){
		//qtss_printf("qtssRTSPReqFilePath.len==0\n");
...
void RTSPRequest::ParseOnDemandSessionId()
{
	StrPtrLen * session = fHeaderDictionary.GetValue(qtssOnDemandSessionId);
	//qtss_printf("session:%s\n",session->Ptr);
	StringParser theSessionParser(session);
	StrPtrLen theSessionID;
	(void)theSessionParser.GetThru(&theSessionID, ';');
	//qtss_printf("qtssOnDemandSession:%s\n",theSessionID.Ptr);

	fHeaderDictionary.SetVal(qtssClientSessionId, &theSessionID);
}

void RTSPRequest::ParseClientSessionID()
{
	StrPtrLen * session = fHeaderDictionary.GetValue(qtssClientSessionId);
	//qtss_printf("session:%s\n",session->Ptr);
	StringParser theSessionParser(session);
	StrPtrLen theSessionID;
	(void)theSessionParser.GetThru(&theSessionID, ';');
	//qtss_printf("qtssClientSessionId:%s\n",theSessionID.Ptr);
...
QTSS_Error RTSPRequest::ParseHeaders(StringParser& parser)
{
	...
	while ((parser.PeekFast() != '\r') && (parser.PeekFast() != '\n')) 
	{
		...
		if(theHeader ==  qtssIllegalHeader && (theKeyWord.Ptr[0]=='K'|| theKeyWord.Ptr[0]=='k')){
			//qtss_printf("keys header\n");
			theHeader = qtssVaryHeader;
		}


		if(theHeader == qtssPlayWithinSetup)
		{
			earlyPlayFlag = 1;
		}		
		...
		//qtss_printf("theHeader:%d,theHeaderVal:%s\n",theHeader,theHeaderVal.GetAsCString());
		...
		switch (theHeader)
		{
		case qtssXregionIDHeader:           ParseXRegionIDHeader();  break;
		case qtssSessionHeader:             ParseSessionHeader(); break;
		case qtssClientSessionId:           ParseClientSessionID(); break;
		case qtssOnDemandSessionId:         ParseOnDemandSessionId(); break;
...
void RTSPRequest::ParseXRegionIDHeader()
{
	isIpqam = true;
	const char *dest="192.168.1.188";
	...
	//const char *dest="192.168.0.102";
	qtss_printf("dest:%s\n",dest);
	fDestinationAddr=SocketUtils::ConvertStringToAddr(dest);
...
void RTSPRequest::ParseTransportHeader()
{
	...
	{	
		...
		(void)theTransParser.ConsumeUntil(&fFirstTransport, ' ');//used in ningxia-wasu
	...
	while (theTransportSubHeader.Len > 0)
	{
		...
		if (!this->ParseNetworkModeSubHeader(&theTransportSubHeader))
		{
			...
			case 'R':   // RTP/AVP/??? Is this TCP or UDP?
				{   
					if ( theTransportSubHeader.EqualIgnoreCase("RTP/AVP/TCP")  || strstr(theTransportSubHeader.Ptr,"TCP")!=NULL)
			...
			case 'M':   //mode sub-header
				{
					if(!strncmp("MP2T/H2221/UDP",theTransportSubHeader.Ptr, strlen("MP2T/H2221/UDP")) || strstr(theTransportSubHeader.Ptr,"MP2T")!=NULL)
					{
						isMPEG2=true;
						qtss_printf("receive new request,mp2t\n");                        

						if( strstr(theTransportSubHeader.Ptr,"TCP")!=NULL)	
						{
							fTransportType = qtssRTPTransportTypeTCP;
							qtss_printf("receive new request,tcp\n"); 
						}      

					}

					if(strstr(theTransportSubHeader.Ptr,"QAM")!=NULL)
					{
						isIpqam =true;
					}
...
void RTSPRequest::ParseAddrSubHeader(StrPtrLen* inSubHeader, StrPtrLen* inHeaderName, UInt32* outAddr)
{
	...
	// CHANGE - Checking to see if transport header format is:
	// Transport: RTP/AVP;multicast;ttl=15;destination=229.41.244.93:5000
	StringParser theClientSubHeaderParser(inSubHeader);
	StrPtrLen thePortStart;
	if(theClientSubHeaderParser.GetThru(&thePortStart,':'))
	{
		StrPtrLen theAddr;
		theSubHeaderParser.ConsumeUntil(&theAddr, ':');
		...
		*outAddr = SocketUtils::ConvertStringToAddr(theAddr.Ptr);
		...
		// Get the port nos.
		theClientSubHeaderParser.ConsumeWhitespace();
		fClientPortA = (UInt16)theClientSubHeaderParser.ConsumeInteger(NULL);
		fClientPortB = fClientPortA + 1;
	}
	...
	{	
		...
		char *p = rindex(theAddr.Ptr,'!');
		if(p)
		{
			StrPtrLen theAddr1(p+1);
			//qtss_printf("Addr:%s\n",theAddr1.Ptr);
			*outAddr = SocketUtils::ConvertStringToAddr(theAddr1.Ptr);
			theAddr1.Ptr[theAddr1.Len] = theTerminator;
		}else
		{		
			*outAddr = SocketUtils::ConvertStringToAddr(theAddr.Ptr);		
			theAddr.Ptr[theAddr.Len] = theTerminator;
