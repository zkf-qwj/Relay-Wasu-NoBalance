    ...
    if (amtInBuffer > 0)
    {
		...
		if (fProtocolLogEnabled)
		{
			...
			if (ProtocolFile != NULL)
			{
				...
				for (UInt32 i =0; i < inNumVectors; i++)
				{	StrPtrLen str((char*)inVec[i].iov_base, (UInt32) inVec[i].iov_len);
					str.PrintStrEOL(NULL,NULL,ProtocolFile);
				}
				::fclose(ProtocolFile);
  			}
 		}*/
...
QTSS_Error RTSPResponseStream::Flush()
{
    ...
    if (amtInBuffer > 0)
    {
        ...
        /*add by cnstream, but not used now
		if (fProtocolLogEnabled)
		{
			char*	filePath = fProtocolLogFileName.Ptr;	  
			FILE*	ProtocolFile = ::fopen(filePath, "a");
			...
			if (ProtocolFile != NULL)
			{
	            ...
	            DateBuffer theDate;
	            memset(&theDate,0,sizeof(DateBuffer));
    	        DateTranslator::UpdateDateBuffer(&theDate, 0); // get the current GMT date and time
		...
		*/
        ...
        if (theLengthSent > 0){
			printf("zlj/add for qingyang send  1\n");
			fTimeoutTask->RefreshTimeout();
		}
            
        if (theLengthSent == amtInBuffer)
        {
			printf("zlj/add for qingyang send  2\n");
			memset(this->GetBufPtr(),0,strlen(this->GetBufPtr()));
            // We were able to send all the data in the buffer. Great. Flush it.
            this->Reset();
            fBytesSentInBuffer = 0;
        }
        ...
        {
			printf("zlj/add for qingyang send  3,error:%s\n",strerror(errno));
