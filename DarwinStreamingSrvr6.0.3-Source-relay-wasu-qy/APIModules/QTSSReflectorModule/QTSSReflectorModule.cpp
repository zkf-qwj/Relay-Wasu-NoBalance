...
static QTSS_AttributeID         sARTSClientRequestPathAttr = qtssIllegalAttrID;
...
QTSS_Error Register(QTSS_Register_Params* inParams)
{
	...
	static char*        sClientRequestPath          = "QTSSClientRequestPath";
	...
	error =QTSS_AddStaticAttribute(qtssClientSessionObjectType, sClientRequestPath, NULL, qtssAttrDataTypeVoidPointer);
	(void)QTSS_IDForAttr(qtssClientSessionObjectType, sClientRequestPath, &sARTSClientRequestPathAttr);
	qtss_printf("sARTSClientRequestPathAttr:%d,err:%d\n",sARTSClientRequestPathAttr,error);
...
QTSS_Error  generate_sdp(char *vid,UInt32 l_callid,char *sdp_buf,Bool16 isCamera,char *src_host,UInt32 *src_port,char *local_host)
{
	...
	char key[1024]={'\0'};
	if(!sRelayModuleClient && isCamera)
	{
	...
	int rc =mem_get(memc,key,multi_addr);
	RLogRequest(DEBUG_ARTS_MODULE,l_callid,"ret:%d",rc );
	...
	RLogRequest(INFO_ARTS_MODULE,l_callid,"multi_addr:%s,multi_host:%s,port:%d",multi_addr,multi_host,port);
	...
	{
		sprintf(sdp_buf+strlen(sdp_buf),"a=range:npt=0.000-36100.0\r\nm=video %d MP2T mpgv\r\na=control:trackID=1\r\n\r\n",port);
		// sprintf(sdp_buf+strlen(sdp_buf),"a=range:npt=0.000-36100.0\r\nm=video %d MP2T mpgv\r\na=control:trackID=0\r\nm=audio 0 MP2T mpgv\r\na=control:trackID=1\r\n\r\n",port);
	}

	if(sARTSTransportType == AVIT_MODULE)
	{
		sprintf(sdp_buf+strlen(sdp_buf),"a=range:npt=0.000-36100.0\r\nm=video %d udp MP2T\r\n\r\n",port);//must with range
	}
...
SInt64 StoreTask::Run()
{
	...
	{
		RLogRequest(INFO_ARTS_MODULE,callid,"kKillEvent");
		return -1;
	}
	if(!stop  )
	{
...
ReflectorSession* DoSessionSetup(QTSS_StandardRTSP_Params* inParams, QTSS_AttributeID inPathType,Bool16 isPush, Bool16 *foundSessionPtr, char** resultFilePath)
{
	...
	QTSS_Error theErr = QTSS_GetValue(theClientSession, sARTSClientSessionAttr, 0, (void *)&l_callid, &l_len);
	...
	QTSS_Error err = QTSS_GetValuePtr(theClientSession, sARTSClientRequestPathAttr, 0,(void**)&requestPath,&l_len);

	RLogRequest(INFO_ARTS_MODULE,0,"will get client:%x",theClientSession);
	if ( err || l_len != sizeof(*requestPath) )
	{
		theErr = QTSS_GetValueAsString(inParams->inRTSPRequest, qtssRTSPReqLocalPath, 0, &theFullPathStr);
		Assert(theErr == QTSS_NoErr);
		char *newFullPath =(char*)malloc(strlen(theFullPathStr) +1);
		strcpy(newFullPath,theFullPathStr);

		err = QTSS_SetValue(theClientSession, sARTSClientRequestPathAttr, 0, (void *)&newFullPath, sizeof(newFullPath));
		RLogRequest(INFO_ARTS_MODULE,0,"set sARTSClientRequestPathAttr err:%d,theClientSession:%x",err,theClientSession);

	}else
		theFullPathStr = *requestPath;
	RLogRequest(INFO_ARTS_MODULE,0,"zlj/add thefull:%s",theFullPathStr);
...
void DoAnnounceAddRequiredSDPLines(QTSS_StandardRTSP_Params* inParams, ResizeableStringFormatter *editedSDP, char* theSDPPtr)
{
	...
	if (!checkedSDPContainer.HasReqLines())
	{
		...
		{ // add s line
			...
			{
				editedSDP->Put("s=MPEG Transport Stream, streamed by the ARTSStreamingServer");
...
void DoDescribeAddRequiredSDPLines(QTSS_StandardRTSP_Params* inParams, ReflectorSession* theSession, QTSS_TimeVal modDate,  ResizeableStringFormatter *editedSDP, StrPtrLen* theSDPPtr)
{
	...
	if (!checkedSDPContainer.HasReqLines())
	{
		...
		{ // add o line
			...
			editedSDP->Put("o=OnewaveUServerEagle "); //zlj/add 0925 for qingyang
			char tempBuff[256]= "";               
			tempBuff[255] = 0;
...
QTSS_Error DoDescribe(QTSS_StandardRTSP_Params* inParams)
{
	...
	RLogRequest(INFO_ARTS_MODULE, 0,"zlj/add describe:%s",theFilepath);
	...
	err =QTSS_GetValuePtr(inParams->inRTSPHeaders, qtssUserAgentHeader,0, (void**)&theRequestAttributes, &attributeLen);
	if(err == QTSS_NoErr)
	{

		if( strstr(theRequestAttributes,"motorola"))
		{          
	...
	{   
		//zlj/add for qingyang 0925
		//editedSDP.Put("m=video 0 MP2T/TCP 33\r\n");
		//editedSDP.Put("m=video 0 MP2T 33\r\n");
		//editedSDP.Put("m=video 0 MP2T 33\r\n");
		...
		//zlj/add for qingyang 0925
		(void)QTSS_AppendRTSPHeader(inParams->inRTSPRequest, qtssCacheControlHeader, "must-revalidate", ::strlen("must-revalidate"));
		QTSSModuleUtils::SendDescribeResponse(inParams->inRTSPRequest, inParams->inClientSession,
				&theDescribeVec[0], 3, sessLen + mediaLen );
...
{   
	...
	if(strstr(inPath->Ptr,"live") != NULL)
	{
		isLive =1;
	}
	...
	if (theSessionRef == NULL)
	{
		...
		{   
			...
			RLogRequest(INFO_ARTS_MODULE,l_callid,"vid:%s,isCamera:%d,sRelayModuleClient:%d,sDataBaseHost:%s",vid,isCamera,sRelayModuleClient,sDataBaseHost);
			char sdp_buf[1024]={'\0'};
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params* inParams)
{
	...
	RLogRequest(DEBUG_ARTS_MODULE, l_callid,"zlj/add full request:%s",theRequestAttributes);

	/*zlj/add for qingyang 0921*/
#if 0
	char *fullstr = "trackID=1 \0";
	char newstr[2048] = {0};                                                                                                                                                                
	char *start = NULL,*end = NULL;
	start = strstr(theRequestAttributes,"*");
	end = strstr(theRequestAttributes,"RTSP");
	int len = 0;
	len = start - theRequestAttributes;
	if (start != NULL && end != NULL)
	{
		qtss_printf("zlj/add  this is qingyang mode!\n");
		strncpy(newstr,theRequestAttributes,len);
		strncpy(newstr+len,fullstr,strlen(fullstr));
		strncpy(newstr+len+strlen(fullstr),end,strlen(end));
		strncpy(theRequestAttributes,newstr,strlen(newstr));
	}
#endif
	...
	RLogRequest(INFO_ARTS_MODULE, l_callid,"theDigitStr:%s",theDigitStr);
	...
	else if(theDigitStr && theSession->SessionTransportType != qtssRTPTransportTypeMPEG2)
	{
	...
	RLogRequest(DEBUG_ARTS_MODULE, l_callid,"!isPush,theTrackID:%d",theTrackID);
	...
	if (theStreamInfo == NULL)
	{
		RLogRequest(DEBUG_ARTS_MODULE, l_callid,"bad TrackID");
		return QTSSModuleUtils::SendErrorResponse(inParams->inRTSPRequest, qtssClientBadRequest,
				sReflectorBadTrackIDErr);
	}
...
QTSS_Error DestroySession(QTSS_ClientSessionClosing_Params* inParams)
{
	...
	ReflectorSession*   theSession = NULL;
	RLogRequest(INFO_ARTS_MODULE,0,"Reflectotr module DestroySession\n");
	qtss_printf("Reflectotr module DestroySession\n");
	...
	{
		RLogRequest(INFO_ARTS_MODULE,0,"callid get failed\n");
		return QTSS_NoErr;
	}
	qtss_printf("get callid ok\n");
	UInt32 theLen = sizeof(theSession);
	...
	if(sessIDCStr && sSupportOtherChannel)
	{
		...
		if(cur_p) 
		{
			...
			if(cur_p->rtpSessObj)
			{           
				QTSS_Teardown(cur_p->rtpSessObj); 
			}            
