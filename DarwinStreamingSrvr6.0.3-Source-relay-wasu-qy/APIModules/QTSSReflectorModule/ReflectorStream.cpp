...
static Bool16  SetUpUDPSocketsPool(QTSServerPrefs* inPrefs)
{
    ...
    {             
        if(strlen(portsStr) >0)
        {
            ...
            qtss_printf("theBindIPAddr:%s\n",theBindIPAddr);
            if(theBindIPAddr )
            {
                sprintf(key,"%s_udp_ports_range",theBindIPAddr);
                mem_set(memc,key,portsStr,2505600);
...
void ReflectorSender::ReflectPackets(SInt64* ioWakeupTime, OSQueue* inFreeQueue)
{
   ...
   // if (fWriteFlag == qtssWriteFlagsIsRTCP)
    ...
    RLogRequest(DEBUG_ARTS_MODULE,0,"fNumBuckets:%d,sBucketSize:%d\n",fStream->fNumBuckets,fStream->sBucketSize);
    for (UInt32 bucketIndex = 0; bucketIndex < fStream->fNumBuckets; bucketIndex++)
    {   
        for (UInt32 bucketMemberIndex = 0; bucketMemberIndex < fStream->sBucketSize; bucketMemberIndex++)
        {    
            ...
            if(theOutput == NULL)
            {
               RLogRequest(DEBUG_ARTS_MODULE,0,"output null,bucketMemberIndex:%d,bucketIndex:%d",bucketMemberIndex,bucketIndex);
            }
            
            if (theOutput != NULL)
            {  
                RLogRequest(DEBUG_ARTS_MODULE,theOutput->callid,"will send packets,bucketMemberIndex:%d,bucketIndex:%d",bucketMemberIndex);  
                ...
                RLogRequest(DEBUG_ARTS_MODULE,theOutput->callid,"packetElem:%x,bucketIndex:%d,fStream->fNumBuckets:%d,bucketMemberIndex:%d,fStream->sBucketSize:%d",packetElem,bucketIndex,fStream->fNumBuckets,bucketMemberIndex,fStream->sBucketSize);
