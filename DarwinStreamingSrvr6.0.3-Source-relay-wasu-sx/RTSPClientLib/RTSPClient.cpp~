    ...
    if (!IsTransactionInProgress())
    {
        ...
        */
        ...
        fmt.PutFmtStr(
                "SETUP %s/%s=%"_U32BITARG_" RTSP/1.0\r\n"
                "CSeq: %"_U32BITARG_"\r\n"
                "%sTransport: MP2T;unicast;client_port=%u-%u\r\n"
                "x-Retransmit: our-retransmit\r\n",               
                fURL.Ptr,fControlID, inTrackID, fCSeq, fSessionID.Ptr, inClientPort, inClientPort + 1);
		if (fBandwidth != 0)
			fmt.PutFmtStr("Bandwidth: %"_U32BITARG_"\r\n", fBandwidth);
    ...
    {   
        ...
        if (fTransportMode == kPushMode)
			fmt.PutFmtStr(
                    "SETUP %s/%s=%"_U32BITARG_" RTSP/1.0\r\n"
                    "CSeq: %"_U32BITARG_"\r\n"
                    "%sTransport: MP2T/TCP;unicast;mode=record;interleaved=%u-%u\r\n"
                    "User-agent: %s\r\n",
            ...
            fmt.PutFmtStr(
                    "SETUP %s/%s=%"_U32BITARG_" RTSP/1.0\r\n"
                    "CSeq: %"_U32BITARG_"\r\n"
                    "%sTransport: MP2T/TCP;unicast;interleaved=%u-%u\r\n"
                    "%sUser-agent: %s\r\n",
