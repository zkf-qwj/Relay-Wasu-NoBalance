                                                ...
                                                UInt32 inSrcIPAddr, UInt16 inSrcPort)
{
    ...
    //qtss_printf("inSrcIPaddr:%d,inSrcPort:%d,inIPAddr:%u,inPort:%d\n",inSrcIPAddr,inSrcPort,inIPAddr,inPort);
    
    //qtss_printf("inSrcPort:%d,inIPAddr:%u,inPort:%d\n",inSrcPort,inIPAddr,inPort);
    //if ((inSrcIPAddr != 0) || (inSrcPort != 0))
    {
        ...
        for (OSQueueIter qIter(&fUDPQueue); !qIter.IsDone(); qIter.Next())
        {
            ...
            qtss_printf("addr:%u,port:%d\n",theElem->fSocketA->GetLocalAddr(),theElem->fSocketA->GetLocalPort());
                ...
                ((inPort == 0) || (theElem->fSocketA->GetLocalPort() == inPort)))
            {
                ...
                {
                    qtss_printf("Get UDP Ports from pool\n");
                    theElem->fRefCount++;
                    return theElem;
                }
...
UDPSocketPair*  UDPSocketPool::CreateUDPSocketPair(UInt32 inAddr, UInt16 inPort)
{
   //try to find an open pair of ports to bind these suckers tooo
    ...
    while ((!foundPair) && (curPort < kHighestUDPPort))
    {
        ...
        if (theErr == OS_NoErr)
        {   //qtss_printf("fSocketA->Bind ok on port%u,fd:%d\n", curPort,theElem->fSocketA->GetSocketFD());
            theErr = theElem->fSocketB->Bind(inAddr, socketBPort);
            if (theErr == OS_NoErr)
            {   //qtss_printf("fSocketB->Bind ok on port%u,fd:%d\n", socketBPort,theElem->fSocketA->GetSocketFD());
                foundPair = true;
