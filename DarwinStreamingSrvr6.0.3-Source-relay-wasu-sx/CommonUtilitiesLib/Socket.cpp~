...
void Socket::Enable_DiffServ(UInt8 TOS){

   UInt8 tos = TOS;
   int tos_len = sizeof(tos);
   int err = ::setsockopt(fFileDesc, IPPROTO_IP, IP_TOS, (char*)&tos, tos_len );
   Assert(err == 0);
}
...
OS_Error Socket::Send(const char* inData, const UInt32 inLength, UInt32* outLengthSent)
{
    ...
    if (err == -1)
    {
        ...
        {
            qtss_printf("[send -1] close fd:%d\n",fFileDesc);
            fState ^= kConnected;//turn off connected state flag 
            this->DontAutoCleanup();
            this->Cleanup();               
        }    
...
OS_Error Socket::WriteV(const struct iovec* iov, const UInt32 numIOvecs, UInt32* outLenSent)
{
    ...
    if (err == -1)
    {
        ...
        // Yes... EAGAIN. Means the socket is now flow-controleld
        int theErr = OSThread::GetErrno();
       // printf("socket(%d):writeV:ERR:%d\n",fFileDesc,theErr);
        if ((theErr != EAGAIN) && (this->IsConnected()))
        {
            fState ^= kConnected;//turn off connected state flag
            qtss_printf("[writev -1]  close fd:%d\n",fFileDesc);
            this->DontAutoCleanup();
            this->Cleanup();            
         }
        return (OS_Error)theErr;
...
OS_Error Socket::Read(void *buffer, const UInt32 length, UInt32 *outRecvLenP)
{
    ...
    do {
       ...
       //qtss_printf("theRecvLen%d,fd:%d\n",theRecvLen,fFileDesc);
    ...
    if (theRecvLen == -1)
    {
        ...
        // Yes... EAGAIN. Means the socket is now flow-controleld
        int theErr = OSThread::GetErrno();
       //qtss_printf("SocketErr:%d\n",theErr);
        if ((theErr != EAGAIN) && (this->IsConnected()))
        {
            fState ^= kConnected;//turn off connected state flag
            qtss_printf("[read -1] close fd:%d\n",fFileDesc);
            this->DontAutoCleanup();
            this->Cleanup();
