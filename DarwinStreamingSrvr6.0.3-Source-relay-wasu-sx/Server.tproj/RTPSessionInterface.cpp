...
{   
    if (inNewRTSPSession != fRTSPSession)
    {
        ...
        if (fRTSPSession != NULL)
        {
            ...
            if(fRTSPSession->NoCleanObjectHolderCount== false  &&  fRTSPSession->szModule== false)
            {               
                //qtss_printf("DecrementObjectHolderCount,fRTSPSession:%x\n",fRTSPSession);           
                fRTSPSession->DecrementObjectHolderCount();
            }else
            {
                 //qtss_printf("fRTSPSession->szModule:%d\n",fRTSPSession->szModule);
            }
         }
       ...
       // qtss_printf("IncrementObjectHolderCount,fRTSPSession:%x\n",fRTSPSession);
    }
}
...
QTSS_Error RTPSessionInterface::DoSessionSetupResponse(RTSPRequestInterface* inRequest)
{
    ...
    char RTSPTimeout[25];
    sprintf(RTSPTimeout,"%d",QTSServerInterface::GetServer()->GetPrefs()->GetRTSPTimeoutInSecs());
    StrPtrLen inTimeout(RTSPTimeout);  
	
    if ( QTSServerInterface::GetServer()->GetPrefs()->GetRTSPTimeoutInSecs() > 0 )  // adv the timeout
        inRequest->AppendSessionHeaderWithTimeout( this->GetValue(qtssCliSesRTSPSessionID), &inTimeout);//QTSServerInterface::GetServer()->GetPrefs()->GetRTSPTimeoutAsString() );
    else
