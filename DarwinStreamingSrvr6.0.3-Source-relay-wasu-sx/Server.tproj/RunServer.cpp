...
//add zlj 20160612
#if 1
class ARTS_License_thread : public OSThread
{
	private:
		virtual void Entry(){
			qtss_printf("--call back function\n");
...
ARTS_License_thread *sARTS_License_thread = NULL;
#endif
...
QTSS_ServerState StartServer(XMLPrefsParser* inPrefsSource, PrefsSource* inMessagesSource, UInt16 inPortOverride, int statsUpdateInterval, QTSS_ServerState inInitialState, Bool16 inDontFork, UInt32 debugLevel, UInt32 debugOptions)
{
	...
	sARTS_License_thread = new ARTS_License_thread();
	if(sARTS_License_thread == NULL){
		qtss_printf("Could not create  license of thread");
		exit(1);
	}
...
#if USE_FLEXM
/*-------------------------------------------Start* * License------------------------------*/
	 printf("\n before   sARTS_License_thread -> Start\n");
	 sARTS_License_thread->Start();
	 printf("\n after   sARTS_License_thread -> Start\n");
#endif
	//
			...
			(theServerState != qtssFatalErrorState))
	{
		...
		if ((sServer->SigIntSet()) || (sServer->SigTermSet()))
		{
			...
			if(sServer->SigIntSet() )
				qtss_printf("sServer->SigIntSet\n");

			if(sServer->SigTermSet())
				qtss_printf("sServer->SigTermSet()\n");
			qtss_printf(" qtssShuttingDownState \n");
