...
QTSS_Error QTSSCallbacks::QTSS_Enable_DiffServ(QTSS_StreamRef inStream, UInt8 TOS)
{

 if (inStream == NULL)
        return QTSS_BadArgument;
    QTSS_Error theErr = ((QTSSStream*)inStream)->Enable_DiffServ(TOS);
                                                            ...
                                                            UInt32 inFlags)
{
    ...
    switch (((RTSPRequestInterface*)inRTSPRequest)->GetMethod())
    {
        ...
        case qtssGetParameterMethod:
            ((RTPSession*)inRTPInfo)->SendGetParameterResponse((RTSPRequestInterface*)inRTSPRequest);
            return QTSS_NoErr;	
        case qtssValidateMethod:
            ((RTPSession*)inRTPInfo)->SendDefaultResponse((RTSPRequestInterface*)inRTSPRequest);
            return QTSS_NoErr;
